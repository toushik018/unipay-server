...
async function run() {
  try {
    ...
    const orderCollection = client.db("toursDB").collection("order");
    ...
    app.post('/users', async (req, res) => {
      const user = req.body;
      const query = { email: user.email, id: user.id };
   ...
   // -------------------------   Payment APIs - Tours -------------
    ...
    app.get("/tourorders", async (req, res) => {
      const result = await tourOrdersCollection.find().toArray();
      res.send(result);
    });
